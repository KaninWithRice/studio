...
import { ChefHat, CheckCircle2, Clock, FileText, PackageCheck, ShoppingBasket, Utensils } from 'lucide-react';
...
const orderStatusOptions: OrderStatus[] = ['New', 'Preparing', 'Ready', 'Served', 'Cancelled'];
    ...
    <Card className="shadow-lg w-full">
      ...
      <CardContent>
        <div className="mb-2 flex items-center gap-2">
          ...
          <span className={`font-semibold ${order.status === 'New' ? 'text-destructive' : order.status === 'Ready' ? 'text-green-600' : 'text-primary'}`}>
            Status: {order.status}
          </span>
        </div>
        ...
        <Separator className="my-2" />
        <p className="font-bold text-lg text-primary">Total: ${order.totalAmount.toFixed(2)}</p>
      </CardContent>
      <CardFooter className="flex flex-col sm:flex-row items-center gap-2">
        ...
        <Button 
            ...
            onClick={() => {/* Potentially add other actions like print ticket */}}
        >
